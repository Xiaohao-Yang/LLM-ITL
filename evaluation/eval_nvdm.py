...
import argparse
import os
from topic_models.nvdm import FeatDataset
from topic_models.nvdm import NVDM # to load model
...
if __name__ == '__main__':
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=args.inference_bs, shuffle=False)

    train_dataset = FeatDataset(train_data)
    train_dataloader = DataLoader(train_dataset, batch_size=args.inference_bs, shuffle=False)
    ...
    for model_epoch in tqdm(model_epochs):
        ...
        doc_topic_train = []
        with torch.no_grad():
            for data_batch, _ in train_dataloader:
                data_batch = data_batch.float().cuda()
                _, _, _, theta = model(data_batch)
